...
/**
 * Serializes all named quarks. Use to serialize the quark states on the server,
 * when using SSR, and send them to the client for hydration.
 *
 * This functions requires `serialize-javascript` package, and as a result can
 * only be used in Node environments.
 */
export const serializeQuarks = () => {
  ...
  return JSON.stringify(
    serialize(
      [...quarkCollection.entries()].map(([name, context]) => [
        name,
        context.value,
      ]),
      { ignoreFunction: true },
...
/**
 * Deserializes named quarks and updates the state of each included named quark
 * with the state from the serialized data.
 */
export const hydrateQuarks = (
  ...
  const data: Array<[string, unknown]> = deserialize(
    JSON.parse(serializedQuarks),
  );
