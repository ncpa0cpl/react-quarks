...
export type QuarkType<Q extends Quark<any, any, any>> = Q extends
  Quark<infer T, any, any> ? T : never;
...
export type Rewrap<T> = T extends Function ? T
  : T extends object ? T extends infer O ? {
        [
          K in keyof O as string extends K ? never
...
export type IsLiteral<S extends string> = S extends `${infer A}${string}` ? true
  : false;
...
export type FinalReturnType<F, MAX extends null[] = [null]> = F extends (
  ...args: any[]
) => infer R ? MAX["length"] extends 10 ? F
